...
export default function MaterialWasteDetailScreen() {
  ...
  if (waste.isLoading) {
    return (
      <>
        <Stack.Screen options={{ title: "Phiếu huỷ" }} />
        <LoadingState />
      </>
    );
  }
  ...
  return (
    <>
      ...
      <Screen refreshing={waste.isRefetching} onRefresh={() => waste.refetch()}>
        <Card>
          <CardBody style={styles.infoCard}>
            <Text style={styles.code}>{data.code}</Text>
            <InfoRow label="Thời điểm huỷ" value={formatDateVN(data.wasteAt)} />
            <InfoRow label="Người tạo" value={data.createdBy?.name ?? "—"} />
            {data.note ? <InfoRow label="Ghi chú" value={data.note} /> : null}
          </CardBody>
        </Card>

        {can("MATERIAL_WASTE", "EDIT") ? (
          <Button
            title="Sửa phiếu"
            variant="secondary"
            fullWidth
            icon={<Ionicons name="pencil" size={16} color={colors.text} />}
            onPress={() => router.push(`/material-waste/${data.id}/edit`)}
          />
        ) : null}
                ...
                <View style={styles.quantities}>
                  <Text style={styles.quantity}>
                    ...
                    <Text style={styles.quantityValue}>
                      {item.wholeQuantity != null ? formatNumber(item.wholeQuantity) : "—"}
                    </Text>
                  </Text>
                  <Text style={styles.quantity}>
                    Lẻ:{" "}
                    <Text style={styles.quantityValue}>
                      {item.looseQuantity != null ? formatNumber(item.looseQuantity) : "—"}
                    </Text>
...
const styles = StyleSheet.create({
  ...
  code: { fontSize: fontSize.xl, fontWeight: "700", color: colors.text, marginBottom: spacing.sm },
