...
function toMaterialRows(transfer?: MaterialTransfer): MaterialRow[] {
  return (transfer?.items ?? []).map((it) => ({
    ...
    costPrice: it.costPrice != null ? String(Number(it.costPrice)) : "",
    note: it.note ?? "",
  }));
}

interface MaterialTransferFormClientProps {
  /** Phiếu hiện có — truyền vào để vào chế độ sửa; để trống là tạo mới. */
  existing?: MaterialTransfer;
}
...
export function MaterialTransferFormClient({ existing }: MaterialTransferFormClientProps) {
  ...
  const updateTransfer = useUpdateMaterialTransfer(existing?.id ?? "");
  ...
  const isPending = isEdit ? updateTransfer.isPending : createTransfer.isPending;
  const backHref = isEdit && existing ? `/material-transfers/${existing.id}` : "/material-transfers";

  return (
    <div className="flex flex-col gap-4">
      <div>
        <Link href={backHref} className="self-start text-sm text-indigo-600 hover:underline">
          ← {isEdit ? "Chi tiết phiếu điều chuyển" : "Danh sách phiếu điều chuyển"}
        </Link>
        <h1 className="mt-2 text-xl font-semibold text-slate-800">{isEdit ? `Sửa phiếu ${existing?.code}` : "Tạo phiếu điều chuyển"}</h1>
      ...
      <div className="flex justify-end gap-2">
        <Button onClick={handleSubmit} disabled={isPending}>
          {isPending ? "Đang lưu..." : isEdit ? "Lưu thay đổi" : "Lưu phiếu điều chuyển"}
        </Button>
      </div>
    </div>
  );
